package com.victor.video.library.drawer;
...
import com.victor.video.library.filter.AFilter;
import com.victor.video.library.filter.CameraFilter;
import com.victor.video.library.filter.GroupFilter;
import com.victor.video.library.filter.NoFilter;
import com.victor.video.library.filter.ProcessFilter;
import com.victor.video.library.filter.WaterMarkFilter;
...
import com.victor.video.library.record.video.TextureMovieEncoder;
import com.victor.video.library.utils.EasyGlUtils;
import com.victor.video.library.utils.MatrixUtils;
...
//        showFilter.setTextureId(textureID);
    ...
    public void onDrawFrame(GL10 gl10) {
        ...
        mBeFilter.draw();
        if (mBeautyFilter != null && App.beautyLevel != 0){
            EasyGlUtils.bindFrameTexture(fFrame[0],fTexture[0]);
            GLES20.glViewport(0,0,mPreviewWidth,mPreviewHeight);
        ...
        if (recordingEnabled){
            ...
            switch (recordingStatus){
                case RECORDING_OFF:
                    int bitRate = 1000000; //2.46 MB
//                    bitRate = 3 * 1024 *1024;//5.55 MB
//                    bitRate = 4 * 1024 *1024;//7.10 MB
//                    bitRate = 5 * 1024 *1024;//9.09 MB
//                    bitRate = 6 * 1024 *1024;//14.81 MB
                    bitRate = 8 * 1024 *1024;//20.47.81 MB 这个值越大录制的视频越清晰
                    ...
                    videoEncoder.startRecording(new TextureMovieEncoder.EncoderConfig(
                            ...
                            bitRate, EGL14.eglGetCurrentContext()));
    ...
    public void setFilter(MagicFilterType type) {
        mSlideFilterGroup.setFilter(type);
    }
    public void setFilter (int filterIndex) {
        mSlideFilterGroup.setFilter(filterIndex);
    }

    public GPUImageFilter getCurFilter () {
        if (mSlideFilterGroup == null) {
            return null;
        }
        return mSlideFilterGroup.getCurFilter();
    }
}
